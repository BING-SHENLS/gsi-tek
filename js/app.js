...
(function(){var c=document.querySelector('.contact'),s=document.querySelector('.contact-slice'),f=document.querySelector('.site-footer');if(!c||!s||!f)return;function fit(){var h=f.getBoundingClientRect().bottom-c.getBoundingClientRect().top;s.style.setProperty('--burst-h',Math.round(h)+'px');s.style.setProperty('--ch',Math.round(c.offsetHeight)+'px')}fit();window.addEventListener('load',fit);window.addEventListener('resize',fit);if('ResizeObserver'in window){var ro=new ResizeObserver(fit);ro.observe(c);ro.observe(f)}})();
(function(){var s=document.querySelector('.contact-slice'),f=document.querySelector('.site-footer');if(!s||!f)return;
var sel='.footer-main>*>*,.footer-main>*:not(:has(*)),.back-top,.footer-bottom>*,.footer-wordmark>i';
function mark(){var mq=matchMedia('(min-width:901px)').matches;var r=s.getBoundingClientRect();f.querySelectorAll(sel).forEach(function(el){var on=false;if(mq&&r.width){var e=el.getBoundingClientRect();var y=(e.top+e.bottom)/2;var t=Math.min(1,Math.max(0,(y-r.top)/r.height));var bx=r.left+r.width*.56*(1-t);var inY=y>=r.top&&y<=r.bottom;on=inY&&e.left>=bx-4;var edge=inY&&!on&&e.right>bx-2&&!el.matches('.footer-wordmark>*');el.classList.toggle('edge-photo',!!(mq&&edge));}el.classList.toggle('on-photo',on)})}
var q=0;function sched(){if(q)return;q=requestAnimationFrame(function(){q=0;mark()})}
mark();window.addEventListener('load',sched);window.addEventListener('resize',sched);if('ResizeObserver'in window){var w=0;new ResizeObserver(function(){var nw=f.offsetWidth+'x'+f.offsetHeight;if(nw!==w){w=nw;sched()}}).observe(f)}})();
